function Calendar() {
	...
	//will hold complete month of log entries
	this.logs = [];
	//will hold max of one log entry (the last) for each day in month
	this.logdays = [];
...
Calendar.prototype.processDates = function() {
	...
	for (var z = 0; z < this.logs.length; z++) {
		var log_date = this.logs[z].date;
		var now = log_date.getDate();
		if (log_date.getMonth() == this.date.getMonth()	// is in the month on display
		  && log_date.getYear() == this.date.getYear()	// is in the year on display
...
Calendar.prototype.change_day = function(target) {
	...
	var month_name = MONTHS[this.date.getMonth()];
	var please_select_msg = "Select a revision for " + this.date.getDate() + " " + month_name;
	replaceChildNodes("cal-revs",
		OPTION({"value" : 'HEAD'}, "HEAD - the most recent version"),
		OPTION({"value" : -1, "selected" : "selected"}, please_select_msg)
	);
	...
	for (var i = 0; i < this.logs.length; i++) {
		var log_entry = this.logs[i];
		if (log_entry.date.getDate() == target) {
			var text = IDE_hash_shrink(log_entry.rev) + " " +
			             log_entry.author + ": " +
			             log_entry.message.slice(0, 20) + " (" +
			             log_entry.date.getHours() + ":" +
			             log_entry.date.getMinutes() + ")";
			appendChildNodes("cal-revs", OPTION({ "value" : log_entry.rev}, text));
		}
	}
